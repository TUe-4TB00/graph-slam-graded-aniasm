...
def optimize(graph, initial_estimate):
    ...
    print("Final result: {}".format(optimizer.error()))
...
def minimize_errors(graph, initial_estimate, pose_options):
    ...
    # True/ground truth poses
    true_poses = {
        1: (0.0, 0.0),      # X(1) at (0, 0)
        2: (2.0, 0.0),      # X(2) at (2, 0)
        3: (4.0, 0.0)       # X(3) at (4, 0)
    }
    list_of_errors = []
    
    for pose_num in [1, 2, 3]:
        optimized = result.atPose2(X(pose_num))
        true_x, true_y = true_poses[pose_num]
        
        # Euclidean distance error
        pos_error = np.sqrt((optimized.x() - true_x)**2 + (optimized.y() - true_y)**2)
        list_of_errors.append(pos_error)
